...
ssize_t myread(struct file *file, char __user *usr, size_t val, loff_t *offset)
{
	...
	sprintf(cal,"%x:%x:%x:%x:%x:%x",hour,min,sec,date,month,year);
	copy_to_user(usr,cal,val);
        }
        
	READ_T(command[*offset],msg);
