...
setup_tcp_server_communication(){
    ...
    while(1){
        ...
        { 
            ...
            /* Create a new child process to service this new client*/
            ...
            /*Finally launch the child process to service this particular client. */
            /*fork() returns 0 to child process, and new child process PID to parent process. Hence below if check
             * will be true only for child process, therefore service_client_module() is executed only by child process*/ 

            if(fork() == 0){
                /*After fork(), child process execution flow comes here*/
                printf("New child process created....\n");
                service_client_module(client_info_t);
                printf("child process terminates\n");
                exit(0);
            }
            /*After fork(), parent process execution flow directly comes here because fork returns non zero child process 
             * id to parent*/
        }
